...
namespace Mediapipe.Framework.Formats
{
  public class Image : MpResourceHandle
  {
    ...
    public unsafe Image(ImageFormat.Types.Format format, int width, int height, int widthStep, ReadOnlySpan<byte> pixelData, ImageFrame.Deleter deleter)
      : this(format, width, height, widthStep, spanToBytePtr(pixelData), deleter)
    { }
    ...
    ///   </list>
    /// </remarks>
    public Image(ImageFormat.Types.Format format, int width, int height, int widthStep, ReadOnlySpan<byte> pixelData)
    ...
    private unsafe static IntPtr spanToBytePtr(ReadOnlySpan<byte> span)
    {
        fixed (byte* ptr = span)
        {
            return (IntPtr)ptr;
